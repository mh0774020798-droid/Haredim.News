...
# העתקת קובץ ההגדרות למקום הנכון
COPY --chown=${USER}:${USER} config.json /opt/config/config.json
COPY --chown=${USER}:${USER} config.json /usr/src/app/config.json

USER ${USER}

# --- הפתרון לבעיית הזיכרון ---
# מריצים את הבניה הכבדה עכשיו, בזמן ה-Build של Docker, כשיש ל-Render 8GB של זיכרון
# ובנוסף מייצרים את קובץ החותמת כדי שהשרת ידע שהוא לא צריך לבנות מחדש בעלייה
RUN node ./nodebb build && md5sum package.json | awk '{print $1}' > /opt/config/install_hash.md5
